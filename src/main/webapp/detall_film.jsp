...
<c:if test="${exist}">
    ...
    <div class="d-flex justify-content-center my-2">
        <div class="media">
            <img src="<c:out value="${pelicula.caratula}"/>" class="align-self-start mr-3" style="width:20vw" alt="">
            <div class="media-body">
                <h5 class="mt-0"><c:out value="${pelicula.nom}"/></h5>
                <p>Sinopsis: <c:out value="${pelicula.sinopsis}"/></p>
                <p>Genere: <c:out value="${pelicula.genereFilmEnum}"/></p>
                <p>Estreno: <c:out value="${pelicula.estreno}"/></p>
                ...
                <c:choose>
                <c:when test="${pelicula.edatRec eq 0}">
                    <c:out value="${'<p>Tots els públics'}" escapeXml="false"/>
                    <c:out value="${'<p>Tots els públicos. Divertida per tota la família</p>'}" escapeXml="false"/>
                </c:when>
                <c:when test="${pelicula.edatRec eq 7}">
                    <c:out value="${'<p>Edat recomanora a partir de 7'}" escapeXml="false"/>
                    <c:out value="${'<p>Divertida per tota la família</p>'}" escapeXml="false"/>
                </c:when>
                <c:when test="${pelicula.edatRec eq 16}">
                    <c:out value="${'<p>Edat recomanora a partir de 16'}" escapeXml="false"/>
                    <c:out value="${'<p>Edat recomanora a partir de 16. Pot contenir escenes i vocabulari violent</p>'}"
                           escapeXml="false"/>
                </c:when>
                <c:otherwise>
                    <c:out value="${'<p>Edat recomanora a partir de 18'}" escapeXml="false"/>
                    <c:out value="${'<p>Conté contigut que pot especialment sensible</p>'}" escapeXml="false"/>
                </c:otherwise>
                </c:choose>
                ...
                <p>Sessions: </p>
                    <c:forEach var="hora" varStatus="loop" items="${pelicula.horari}">
                        <c:out value="${hora.value.getHoraInici()}"/> <%--${'<p> Hora Final: ' + hora.value.getHoraFinal() + '</p>'}
                        <%-- Dibuix de n "," --%>
